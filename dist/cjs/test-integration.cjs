...
<root xmlns="http://example.com">
  ...
  `.trim();
    ...
    const parser1 = new index_1.LuciformXMLParser(testXML);
    ...
    console.log('\n📝 Test 2: Parser de compatibilité');
    const parser2 = new migration_1.LuciformXMLParserCompat(testXML);
    ...
    for (let i = 0; i < iterations; i++) {
        new index_1.LuciformXMLParser(testXML).parse();
    }
    const duration1 = Date.now() - start1;
    const start2 = Date.now();
    for (let i = 0; i < iterations; i++) {
        new migration_1.LuciformXMLParserCompat(testXML).parse();
    }
